...
export const SummaryStock = ({ selectedMonth, selectedYear, warehouseId }) => {
  const [summaryTotalStock, setSummaryTotalStock] = useState([]);

  const fetchSummaryTotalStock = async () => {
    try {
      const response = await getSummaryTotalStock({ warehouseId, month: selectedMonth, year: selectedYear });
      if (response.ok) {
  ...
  }, [warehouseId, selectedMonth, selectedYear]);
  return (
