...
const AddressListModal = ({ isOpen, onClose, username, handleSetDefault, onSuccess }) => {
      ...
      <ModalContent>
        ...
        <ModalBody className="scrollbar-hide">
              ...
              <div className="border border-gray-300 rounded-lg p-5 mt-5">
                <div className="flex flex-col justify-between">
                  <div className="flex flex-row">
                    <div className="w-[30%]">
                      <p className="text-md text-gray-600">{`${defaultAddress.firstName.charAt(0).toUpperCase()}${defaultAddress.firstName.slice(1)} ${defaultAddress.lastName.charAt(0).toUpperCase()}${defaultAddress.lastName.slice(1)}`}</p>
                    </div>
                  ...
                  <div className="flex justify-between items-center mt-2">
                    <div className="flex items-center gap-2 font-bold"> <PiTagFill/> Default</div>
                  </div>
                </div>
              </div>
            ))}
          <div className="flex flex-col mt-5">
            ...
            {selectedAddress && <SetDefaultAddressModal isOpen={defaultAddressModal} onClose={() => setDefaultAddressModal(false)} addressData={selectedAddress} userId={userId} onSuccess={()=> fetchAddress()} />}
          </div>
        </ModalBody>
      </ModalContent>
    </Modal>
